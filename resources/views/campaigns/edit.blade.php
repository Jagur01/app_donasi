...
<div class="container">
    ...
    <form action="{{ route('campaigns.update', $campaign->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="number" name="goal_amount" id="goal_amount" class="form-control" value="{{ old('goal_amount', $campaign->goal_amount) }}" required>
            @error('goal_amount')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="date" name="expired" id="expired" class="form-control" value="{{ old('expired', $campaign->expired) }}" required>
            @error('expired')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            @if ($campaign->file_qr)
                ...
                <div>
                    <img src="{{ asset('storage/' . $campaign->file_qr) }}" alt="{{ $campaign->title }}" style="max-width: 200px;">
                </div>
            @endif
            @error('file_qr')
                <div class="text-danger">{{ $message }}</div>
        ...
        <div class="mb-3">
            ...
            @if ($campaign->image)
                <small class="text-muted">Gambar Saat Ini:</small>
                <div>
                    <img src="{{ asset('storage/' . $campaign->image) }}" alt="{{ $campaign->title }}" style="max-width: 200px;">
                </div>
            @endif
            @error('image')
                <div class="text-danger">{{ $message }}</div>
